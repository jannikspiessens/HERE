...
use crate::util::{gen_random, contains_range, ZZbig};
...
pub trait MatrixMul: Clone {
    ...
    // number of bits
    fn size(&self) -> u64 {
        let bitsize = ZZbig.abs_log2_ceil(&self.ring().characteristic(ZZbig).unwrap()).unwrap();
        (self.columns() as u64)*(self.rows() as u64)*(bitsize as u64)
    }
}
